...
export default function Model({ content, name, imageSrc, url }) {
  ...
  const model = modal === "true" && pathname === "/dashboard";
  return (
    <>
      {model && (
        <dialog className="fixed left-0 top-0 h-full w-full bg-black bg-opacity-50 z-50 overflow-auto backdrop-blur flex justify-center items-center ">
